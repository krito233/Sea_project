<template>
  <div style="width: 100%;max-height: 70vh;text-align: center">
    ...
    <button @click="tiptip=1">单点调整</button>
    <button @click="tiptip=2">三点调整</button>
    <button @click="tiptip=3">整体调整</button>
...
export default {
  ...
  methods: {
    ...
    drawLine () {
      ...
      axios({
        ...
        axios({
          ...
          if (res1.data.data === '') {
            for (var i = 0; i <= 23; i++) {
              _this.data_a[i] = new Array()
              _this.data_a[i][0] = i
              _this.data_a[i][1] = eval(res.data.data[i].bg)
            }
          } else {
            for (var i = 0; i <= 23; i++) {
              _this.data_a[i] = new Array()
              _this.data_a[i][0] = i
              _this.data_a[i][1] = eval(res1.data.data[i].bg)
            }
            myChart.setOption({
              series: [{
                id: 'a',
        ...
        function updatePosition () {
          myChart.setOption({
            graphic: echarts.util.map(_this.data_a, function (item, dataIndex) {
              return {
                position: myChart.convertToPixel('grid', item)
              }
            })
        ...
        function onPointDragging (dataIndex, dx, dy) {
          var x_smy = _this.data_a[dataIndex][0], y_smy = _this.data_a[dataIndex][1]
          if (_this.tiptip === 3) {
            for (var i = _this.data_a.length - 1; i >= 0; i--) {
              if ((_this.data_a[i][0] >= xs && _this.data_a[i][0] <= xe && _this.data_a[i][1] >= ys && _this.data_a[i][1] <= ye) || xs == undefined) {
                _this.data_a[i][1] = _this.data_a[i][1] + myChart.convertFromPixel('grid', [0, dx.offsetY])[1] - y_smy
              }
            }
            // Update data
          } else if (_this.tiptip === 2) {
            _this.data_a[dataIndex][1] = _this.data_a[dataIndex][1] + myChart.convertFromPixel('grid', [0, dx.offsetY])[1] - y_smy
            if (dataIndex > 0 && dataIndex < 23) {
              _this.data_a[dataIndex - 1][1] = _this.data_a[dataIndex - 1][1] + (myChart.convertFromPixel('grid', [0, dx.offsetY])[1] - y_smy) * 0.7
              _this.data_a[dataIndex + 1][1] = _this.data_a[dataIndex + 1][1] + (myChart.convertFromPixel('grid', [0, dx.offsetY])[1] - y_smy) * 0.7
            }
    ...
    submit () {
      var jarr = []
      for (var i = 0; i <= 23; i++) {
        // console.log(this.data.data.data[1])
        // console.log(this.data_a)
        var json1 = {
          tm: this.data.data.data[i].tm,
          zq: this.data.data.data[i].zq,
          lx: this.data.data.data[i].lx,
          bg: parseInt(this.data_a[i][1] * 100) / 100,
          stcd: this.data.data.data[i].stcd,
          type: '1'
        }
        jarr.push(json1)
      }
      console.log(JSON.stringify(jarr))
      console.log(this.data_a[2][1])
      axios({
        url: moren + '/thirdparty/tpdata/correctHaiLang.do',
        methods: 'get',
        params: {data: JSON.stringify(jarr)}
